...
SERIAL_PORT = "/dev/tty.usbmodem1101"  # Modifier selon votre configuration
BAUD_RATE = 115200
IMG_PATH = "./images_box/27672263447541_image.jpg"  # Chemin de l'image à envoyer
OUTPUT_FOLDER = "./images_box"  # Dossier pour stocker les résultats
...
def send_image_to_serial(image_path, ser):
    """Envoie une image à l'ESP32 via le port série."""
    try:
        # Charger l'image et la redimensionner à 240x240
        ...
        #ser.write(image_data)  # Envoi de l'image binaire
...
if __name__ == "__main__":
    ...
    try:
        ...
        # Étape 1 : Envoyer 3 pour envoyer l'image
        if not send_command(ser, "3", "1"):
            print("Erreur : L'inférence a échoué ou aucune réponse reçue.")
            ser.close()
            exit()
        else:
            send_image_to_serial(IMG_PATH, ser)
